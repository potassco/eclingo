...
from eclingo.parsing.transformers.theory_parser_epistemic import parse_epistemic_literals_negations, preapend_prefix, parse_epistemic_literals_elements
...
class TestTheoryRemoveNegationsKRules(unittest.TestCase):
    ...
    def assert_ast(self, input, expected, aux_rules=None):
        ...
        def fun(stm):
            ...
            stm = preapend_prefix(stm, "u")
            stm, aux_rules, replacement = parse_epistemic_literals_negations(stm)
    ...
    def test_13(self):
        ...
        expected = """
        #program base.
        #false :- &k { not_u_a(X) :  }; u_b(X); not u_c(X).
        """
    ...
    def test_14(self):
        ...
        expected = """
        #program base.
        #false :- &k { not_u_a(X) :  }; u_b(X); not u_c(X); &k{ sn_u_d(X) : }.
        """
        aux_rules = """[
            sn_u_d(V0) :- -u_d(V0). ,
            not_u_a(X) :- u_b(X); not u_c(X); &k{ sn_u_d(X) : }; not u_a(X).
        ]"""
        self.assert_ast(program, expected, aux_rules)