...
class _VarCollector(Visitor):
    """
    Helper to gather variables in an _AST.
    """
...
def collect_variables(x):
    """
    Get all variables in the given _AST.
    """
    variables = {}
...
class _TermUnpooler(Visitor):
    ...
    def visit_UnaryOperation(self, x):
        ...
        """
        return [_ast.UnaryOperation(x.location, x.operator, y) for y in self.visit(x.term)]

    def visit_BinaryOperation(self, x):
        """
        BinaryOperation(location: Location, operator: BinaryOperator, left: term, right: term)
        """
        return [_ast.BinaryOperation(x.location, x.operator, l, r) for l, r in product(self.visit(x.left), self.visit(x.right))]

    def visit_Interval(self, x):
        """
        Interval(location: Location, left: term, right: term)
        """
        return [_ast.Interval(x.location, l, r) for l, r in product(self.visit(x.left), self.visit(x.right))]

    def visit_Function(self, x):
        """
        Function(location: Location, name: str, arguments: term*, external: bool)
        """
        return [_ast.Function(x.location, x.name, args, x.external)
...
class _AtomUnpooler(Visitor):
    ...
    def visit_Comparison(self, x):
        ...
        """
        return [_ast.Comparison(x.comparison, l, r)
    ...
    def visit_SymbolicAtom(self, x):
        ...
        """
        return [_ast.SymbolicAtom(y) for y in unpool_term(x.term)]
...
def unpool_literal(x):
    ...
    """
    if x.type != _ast.ASTType.Literal:
        raise ValueError("literal expected")

    return [_ast.Literal(x.location, x.sign, y) for y in _AtomUnpooler().visit(x.atom)]
...
def unpool_theory_atom(x):
    ...
    x.elements = list(chain.from_iterable(
        (_ast.TheoryAtomElement(element.tuple, condition)
         for condition in unpool_list_with(unpool_literal, element.condition))
        for element in x.elements))

    return _ast.TheoryAtom(x.location, x.term, x.elements, x.guard)
